  ...
  .directive('d3Geojson', function () {
    ...
    var directiveDefinitionObject = {
      ...
      link: function (scope, element, attrs) {
        ...
        // var dataSource = scope.geo;

        function highlightFeature(d) {
          console.log(d);
          // d3.select(this).style({fill: '#FAAE0A', stroke: '#F08C00', opacity:'0.5', 'stroke-width':'3px'}); }
        }
        ...
        d3.json(dataSource, function (data) {
          ...
          offset = [canvasWidth - (bounds[0][0] + bounds[1][0]) / 2, canvasHeight - (bounds[0][1] + bounds[1][1]) / 2];
          ...
          var areas = group.append('path').attr('d', path).attr('class', 'area')
